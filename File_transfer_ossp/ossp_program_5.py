...
def start_download_server(file_path, **kwargs):
    ...
    Args:
        curr_local (str): The file path to serve.
...
class QrWindow(QtWidgets.QMainWindow, form_class):
    ...
    # We are disabling the cursor so that the output looks cleaner
            ...
            if args.receive:
                start_upload_server(file_path=self.curr_local, debug=args.debug, custom_port=args.port, ip_addr=args.ip_addr, auth=args.auth)
            else:
                start_download_server(
                    self.curr_local,
                    debug=args.debug,
